    ...
    for i in range(nb_lines - 2, -1, -1):
        for j in range(nb_stacks):
            if 4 * j + 1 < len(lines[i]) and lines[i][4 * j + 1] != " ":
                ...
                print(f"Pushing {lines[i][4 * j + 1]} to stack {j}")
    print(stacks)
    return stacks
...
with open("./input.txt", 'r') as f:
    ...
    for instruction in instructions:
        print(f"Moving {instruction.how_many} from {instruction.origin} to {instruction.destination}")
        print(f"Before : {stacks[instruction.origin - 1]} | {stacks[instruction.destination - 1]}")
        stacks[instruction.destination - 1].extend(stacks[instruction.origin - 1][-instruction.how_many:])
        stacks[instruction.origin - 1] = stacks[instruction.origin - 1][:-instruction.how_many]
        print(f"After  : {stacks[instruction.origin - 1]} | {stacks[instruction.destination - 1]}")
