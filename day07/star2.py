...
def compute_smallest_deletable_folder(folder_structure: INode, threshold: int):
    deletable_size = 70000000
    if folder_structure.type == NodeType.FILE:
        return 70000000
    else:
        for child in folder_structure.children:
            deletable_size = min(compute_smallest_deletable_folder(child, threshold), deletable_size)

        if threshold <= folder_structure.size <= deletable_size:
...
with open("./input.txt", "r") as f:
    ...
    empty_space = 70000000 - folder_structure.size
    threshold = 30000000 - empty_space
    print(empty_space, threshold)

    print(compute_smallest_deletable_folder(folder_structure, threshold))
